package com.gtocore.data.recipe.classified;

import com.gtocore.api.data.tag.GTOTagPrefix;
import com.gtocore.common.data.GTOItems;
import com.gtocore.common.data.GTOMaterials;

import com.gregtechceu.gtceu.api.data.chemical.material.MarkerMaterials;
import com.gregtechceu.gtceu.api.data.tag.TagPrefix;
import com.gregtechceu.gtceu.api.fluids.store.FluidStorageKeys;
import com.gregtechceu.gtceu.common.data.GTItems;
import com.gregtechceu.gtceu.common.data.GTMaterials;
import com.gregtechceu.gtceu.data.recipe.CustomTags;

import static com.gtocore.common.data.GTORecipeTypes.NANO_FORGE_RECIPES;

final class NanoForge {

    public static void init() {
        NANO_FORGE_RECIPES.recipeBuilder("gold_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Yellow)
                .inputItems(TagPrefix.block, GTMaterials.Gold, 8)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 16)
                .inputFluids(GTMaterials.NaquadahEnriched, 2000)
                .inputFluids(GTMaterials.SolderingAlloy, 20000)
                .inputFluids(GTMaterials.Bismuth, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Gold)
                .EUt(491520)
                .duration(2000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("osmium_nanites")
                .notConsumable(TagPrefix.lens, GTMaterials.Diamond)
                .inputItems(TagPrefix.block, GTMaterials.Osmium, 8)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 32)
                .inputFluids(GTMaterials.Naquadria, 2000)
                .inputFluids(GTMaterials.Hafnium, 8000)
                .inputFluids(GTMaterials.SolderingAlloy, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Osmium)
                .EUt(491520)
                .duration(4000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("infuscolium_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Pink)
                .inputItems(TagPrefix.block, GTOMaterials.Infuscolium, 8)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 64)
                .inputItems(GTItems.HIGHLY_ADVANCED_SOC, 32)
                .inputFluids(GTMaterials.Neutronium, 8000)
                .inputFluids(GTOMaterials.UuAmplifier, 4000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 40000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.Infuscolium)
                .EUt(7864320)
                .duration(2000)
                .addData("nano_forge_tier", 2)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("draconium_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Purple)
                .inputItems(TagPrefix.block, GTOMaterials.Draconium, 8)
                .inputItems(GTItems.HIGHLY_ADVANCED_SOC_WAFER, 32)
                .inputItems(GTOItems.OPTICAL_RAM_WAFER, 32)
                .inputItems(GTOItems.SIMPLE_OPTICAL_SOC, 32)
                .inputItems(GTOItems.EXOTIC_PROCESSING_CORE, 8)
                .inputFluids(GTMaterials.UUMatter, 40000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 80000)
                .inputFluids(GTOMaterials.SuperMutatedLivingSolder, 80000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.Draconium)
                .EUt(7864320)
                .duration(16000)
                .addData("nano_forge_tier", 2)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("spacetime_nanites")
                .notConsumable(TagPrefix.lens, GTMaterials.Sapphire)
                .notConsumable(GTOItems.QUANTUM_ANOMALY.asItem())
                .inputItems(TagPrefix.block, GTOMaterials.SpaceTime, 8)
                .inputItems(GTOItems.EIGENFOLDED_KERR_MANIFOLD, 4)
                .inputItems(GTOItems.SUPRACAUSAL_RAM_WAFER, 16)
                .inputItems(GTOItems.SUPRACAUSAL_PROCESSING_CORE, 8)
                .inputFluids(GTMaterials.UUMatter, 80000)
                .inputFluids(GTOMaterials.Infinity, 40000)
                .inputFluids(GTOMaterials.TemporalFluid, 40000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.SpaceTime)
                .EUt(125829120)
                .duration(8000)
                .addData("nano_forge_tier", 3)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("neutronium_nanites")
                .notConsumable(TagPrefix.lens, GTMaterials.NetherStar)
                .inputItems(TagPrefix.block, GTMaterials.Neutronium, 8)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 64)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 32)
                .inputFluids(GTMaterials.Neutronium, 4000)
                .inputFluids(GTOMaterials.UuAmplifier, 2000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Neutronium)
                .EUt(491520)
                .duration(32000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("naquadah_nanites")
                .notConsumable(TagPrefix.lens, GTMaterials.Emerald)
                .inputItems(TagPrefix.block, GTMaterials.Naquadah, 8)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 16)
                .inputFluids(GTMaterials.Naquadria, 8000)
                .inputFluids(GTOMaterials.UuAmplifier, 2000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Naquadah)
                .EUt(491520)
                .duration(16000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("carbon_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Lime)
                .inputItems(TagPrefix.block, GTMaterials.Carbon, 64)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 64)
                .inputFluids(GTMaterials.SolderingAlloy, 20000)
                .inputFluids(GTMaterials.Lubricant, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Carbon, 64)
                .EUt(491520)
                .duration(4000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("starmetal_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Red)
                .inputItems(TagPrefix.block, GTOMaterials.Starmetal, 8)
                .inputItems(GTItems.HIGHLY_ADVANCED_SOC, 64)
                .inputItems(GTItems.HIGHLY_ADVANCED_SOC, 64)
                .inputItems(TagPrefix.gemExquisite, GTMaterials.Glass, 64)
                .inputItems(TagPrefix.gemExquisite, GTMaterials.Amethyst, 64)
                .inputFluids(GTMaterials.UUMatter, 40000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 80000)
                .inputFluids(GTOMaterials.SuperMutatedLivingSolder, 80000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.Starmetal)
                .EUt(7864320)
                .duration(8000)
                .addData("nano_forge_tier", 2)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("silver_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.LightGray)
                .inputItems(TagPrefix.block, GTMaterials.Silver, 8)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 16)
                .inputFluids(GTMaterials.NaquadahEnriched, 2000)
                .inputFluids(GTMaterials.SolderingAlloy, 20000)
                .inputFluids(GTMaterials.Bismuth, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Silver)
                .EUt(491520)
                .duration(2000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("orichalcum_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Magenta)
                .inputItems(TagPrefix.block, GTOMaterials.Orichalcum, 8)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 64)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 64)
                .inputFluids(GTMaterials.Neutronium, 8000)
                .inputFluids(GTOMaterials.UuAmplifier, 4000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 40000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.Orichalcum)
                .EUt(491520)
                .duration(32000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("iridium_nanites")
                .notConsumable(TagPrefix.lens, GTMaterials.Glass)
                .inputItems(TagPrefix.block, GTMaterials.Iridium, 8)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 32)
                .inputFluids(GTMaterials.Naquadria, 2000)
                .inputFluids(GTMaterials.Hafnium, 8000)
                .inputFluids(GTMaterials.SolderingAlloy, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Iridium)
                .EUt(491520)
                .duration(4000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("black_dwarf_mtter_nanites")
                .notConsumable(TagPrefix.lens, GTMaterials.Ruby)
                .inputItems(TagPrefix.block, GTOMaterials.BlackDwarfMatter, 8)
                .inputItems(GTOItems.COSMIC_PROCESSING_UNIT_CORE, 8)
                .inputFluids(GTMaterials.UUMatter, 40000)
                .inputFluids(GTOMaterials.Neutron, 40000)
                .inputFluids(GTOMaterials.CosmicElement, 40000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.BlackDwarfMatter)
                .EUt(125829120)
                .duration(4000)
                .addData("nano_forge_tier", 3)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("copper_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Brown)
                .inputItems(TagPrefix.block, GTMaterials.Copper, 8)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 8)
                .inputFluids(GTMaterials.Naquadah, 2000)
                .inputFluids(GTMaterials.SolderingAlloy, 10000)
                .inputFluids(GTMaterials.Bismuth, 10000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Copper)
                .EUt(491520)
                .duration(2000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("rhenium_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Cyan)
                .inputItems(TagPrefix.block, GTMaterials.Rhenium, 8)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 64)
                .inputFluids(GTMaterials.Naquadria, 2000)
                .inputFluids(GTOMaterials.UuAmplifier, 2000)
                .inputFluids(GTMaterials.SolderingAlloy, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Rhenium)
                .EUt(491520)
                .duration(8000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("iron_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Gray)
                .inputItems(TagPrefix.block, GTMaterials.Iron, 8)
                .inputItems(GTItems.SYSTEM_ON_CHIP, 8)
                .inputFluids(GTMaterials.Naquadah, 2000)
                .inputFluids(GTMaterials.SolderingAlloy, 10000)
                .inputFluids(GTMaterials.Bismuth, 10000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Iron)
                .EUt(491520)
                .duration(2000)
                .addData("nano_forge_tier", 1)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("enderium_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Green)
                .inputItems(TagPrefix.block, GTOMaterials.Enderium, 8)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 64)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 64)
                .inputFluids(GTMaterials.Neutronium, 8000)
                .inputFluids(GTOMaterials.UuAmplifier, 4000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 40000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.Enderium)
                .EUt(7864320)
                .duration(2000)
                .addData("nano_forge_tier", 2)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("transcendent_metal_nanites")
                .notConsumable(GTOItems.NON_LINEAR_OPTICAL_LENS.asItem())
                .notConsumable(GTOItems.HYPERCUBE.asItem())
                .inputItems(GTOTagPrefix.NANITES, GTMaterials.Rhenium)
                .inputItems(TagPrefix.block, GTOMaterials.TranscendentMetal, 8)
                .inputItems(GTOItems.RECURSIVELY_FOLDED_NEGATIVE_SPACE, 8)
                .inputItems(CustomTags.MAX_CIRCUITS)
                .inputFluids(GTMaterials.UUMatter, 80000)
                .inputFluids(GTOMaterials.RawStarMatter.getFluid(FluidStorageKeys.PLASMA, 40000))
                .inputFluids(GTOMaterials.SpatialFluid, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.TranscendentMetal)
                .EUt(125829120)
                .duration(16000)
                .addData("nano_forge_tier", 3)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("eternity_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Blue)
                .notConsumable(GTOItems.QUANTUM_ANOMALY.asItem())
                .notConsumable(GTOItems.ETERNITY_CATALYST.asItem())
                .inputItems(GTOTagPrefix.NANITES, GTMaterials.Neutronium)
                .inputItems(TagPrefix.block, GTOMaterials.Eternity, 8)
                .inputItems(GTOItems.CLOSED_TIMELIKE_CURVE_COMPUTATIONAL_UNIT, 8)
                .inputFluids(GTOMaterials.SpatialFluid, 80000)
                .inputFluids(GTOMaterials.ExcitedDtsc, 80000)
                .inputFluids(GTOMaterials.PrimordialMatter, 80000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.Eternity)
                .EUt(125829120)
                .duration(64000)
                .addData("nano_forge_tier", 3)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("cosmic_neutronium_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Black)
                .inputItems(TagPrefix.block, GTOMaterials.CosmicNeutronium, 8)
                .inputItems(GTOItems.SIMPLE_OPTICAL_SOC, 32)
                .inputItems(GTOItems.EXOTIC_WAFER, 32)
                .inputItems(GTOItems.COSMIC_RAM_WAFER, 16)
                .inputItems(GTOItems.COSMIC_PROCESSING_UNIT_CORE, 8)
                .inputFluids(GTMaterials.UUMatter, 40000)
                .inputFluids(GTOMaterials.CrystalMatrix, 40000)
                .inputFluids(GTOMaterials.CosmicMesh.getFluid(FluidStorageKeys.LIQUID, 40000))
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.CosmicNeutronium)
                .EUt(125829120)
                .duration(8000)
                .addData("nano_forge_tier", 3)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("vibranium_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.LightBlue)
                .inputItems(TagPrefix.block, GTOMaterials.Vibranium, 8)
                .inputItems(GTItems.HIGHLY_ADVANCED_SOC, 64)
                .inputItems(GTItems.HIGHLY_ADVANCED_SOC, 64)
                .inputFluids(GTMaterials.UUMatter, 20000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 40000)
                .inputFluids(GTOMaterials.SuperMutatedLivingSolder, 40000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.Vibranium)
                .EUt(7864320)
                .duration(4000)
                .addData("nano_forge_tier", 2)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("white_dwarf_mtter_nanites")
                .notConsumable(TagPrefix.lens, GTMaterials.Ruby)
                .inputItems(TagPrefix.block, GTOMaterials.WhiteDwarfMatter, 8)
                .inputItems(GTOItems.COSMIC_PROCESSING_UNIT_CORE, 8)
                .inputFluids(GTMaterials.UUMatter, 40000)
                .inputFluids(GTOMaterials.Neutron, 40000)
                .inputFluids(GTOMaterials.CosmicElement, 40000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.WhiteDwarfMatter)
                .EUt(125829120)
                .duration(4000)
                .addData("nano_forge_tier", 3)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("uruium_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.LightBlue)
                .inputItems(TagPrefix.block, GTOMaterials.Uruium, 8)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 64)
                .inputItems(GTItems.HIGHLY_ADVANCED_SOC, 64)
                .inputFluids(GTMaterials.UUMatter, 20000)
                .inputFluids(GTOMaterials.MutatedLivingSolder, 40000)
                .inputFluids(GTOMaterials.SuperMutatedLivingSolder, 40000)
                .outputItems(GTOTagPrefix.NANITES, GTOMaterials.Uruium)
                .EUt(7864320)
                .duration(4000)
                .addData("nano_forge_tier", 2)
                .save();

        NANO_FORGE_RECIPES.recipeBuilder("glowstone_nanites")
                .notConsumable(TagPrefix.lens, MarkerMaterials.Color.Orange)
                .inputItems(TagPrefix.block, GTMaterials.Glowstone, 64)
                .inputItems(GTItems.ADVANCED_SYSTEM_ON_CHIP, 64)
                .inputFluids(GTOMaterials.UuAmplifier, 10000)
                .inputFluids(GTMaterials.SolderingAlloy, 20000)
                .inputFluids(GTMaterials.Lubricant, 20000)
                .outputItems(GTOTagPrefix.NANITES, GTMaterials.Glowstone, 64)
                .EUt(491520)
                .duration(16000)
                .addData("nano_forge_tier", 1)
                .save();
    }
}
